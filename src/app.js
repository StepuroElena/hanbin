...
.add-btn {
  ...
  background: rgba(201,123,138,0.2); border: 1px solid var(--color-border);
  color: var(--color-rose); display: flex; align-items: center; justify-content: center;
  cursor: pointer; transition: var(--transition-fast);
}
...
.avatar--logged-in {
  ...
  border: 2px solid rgba(201,123,138,0.4); color: var(--color-text);
}
.avatar--guest {
  background: rgba(255,255,255,0.06);
  border: 1px dashed rgba(201,123,138,0.45); color: rgba(201,123,138,0.7);
}
.avatar--guest:hover {
...
.stats-grid { display: grid; grid-template-columns: 1fr 1fr 1fr 1.6fr; gap: 16px; align-items: stretch; }
.stat-card { padding: 28px 24px; position: relative; overflow: hidden; }
.stat-card::before {
  content: ''; position: absolute; top: -40px; right: -40px;
  width: 120px; height: 120px; border-radius: 50%;
  background: radial-gradient(circle, rgba(201,123,138,0.12), transparent 70%); pointer-events: none;
}
.stat-card:hover { transform: translateY(-4px); }
.stat-label { font-size: 11px; letter-spacing: 0.2em; text-transform: uppercase; color: var(--color-text-muted); margin-bottom: 10px; }
.stat-number { font-family: var(--font-display); font-size: 58px; font-weight: 300; line-height: 1; color: var(--color-text); letter-spacing: -0.02em; display: inline-block; }
.stat-number--shimmer {
...
.quote-card::after { content: '✦'; position: absolute; bottom: 16px; right: 24px; font-size: 48px; color: rgba(201,123,138,0.1); }
.quote-emoji { font-size: 28px; margin-bottom: 12px; }
.quote-text { font-family: var(--font-display); font-size: 22px; font-weight: 400; font-style: italic; line-height: 1.4; color: var(--color-text); margin-bottom: 12px; }
.quote-sub { font-size: 12px; color: var(--color-rose); letter-spacing: 0.15em; text-transform: uppercase; }

/* ── Filters ── */
.filters-row { display: flex; align-items: center; gap: 10px; margin-bottom: 32px; flex-wrap: wrap; animation: fadeUp 0.7s 0.1s ease both; }
.filter-chip {
  padding: 7px 16px; border-radius: 30px;
  border: 1px solid var(--color-border); background: rgba(255,255,255,0.04);
  color: var(--color-text-muted); font-size: 12px; letter-spacing: 0.05em;
...
.see-all {
  font-size: 12px; color: var(--color-rose); cursor: pointer;
  letter-spacing: 0.1em; text-transform: uppercase; opacity: 0.8;
  transition: opacity 0.2s; background: none; border: none; font-family: var(--font-body);
}
...
.watching-card {
  ...
  transition: var(--transition-normal); cursor: pointer; backdrop-filter: blur(8px);
}
.watching-card:hover { transform: translateY(-6px); border-color: rgba(201,123,138,0.35); box-shadow: var(--shadow-card); }
.card-cover { position: relative; aspect-ratio: 2/3; overflow: hidden; background: rgba(255,255,255,0.04); }
...
.card-cover-overlay { position: absolute; inset: 0; background: linear-gradient(to top, rgba(45,15,42,0.95) 0%, transparent 55%); }
...
.btn-add-empty {
  ...
  border: 1px solid var(--color-rose); background: rgba(201,123,138,0.1);
  color: var(--color-rose); cursor: pointer; font-family: var(--font-body);
  font-size: 13px; transition: var(--transition-fast);
}
.btn-add-empty:hover { background: rgba(201,123,138,0.2); }
.loading-dots { padding: 40px; text-align: center; color: var(--color-text-muted); font-size: 13px; animation: pulse 1.5s infinite; }
...
.btn-login-header {
  ...
  border: 1px solid rgba(201,123,138,0.45); border-radius: 40px;
  color: var(--color-text); font-family: var(--font-body); font-size: 13px;
  letter-spacing: 0.05em; cursor: pointer; transition: var(--transition-normal); white-space: nowrap;
}
.btn-login-header svg { color: var(--color-rose); }
.btn-login-header:hover {
  background: linear-gradient(135deg, rgba(201,123,138,0.4), rgba(74,25,66,0.6));
  border-color: var(--color-rose); transform: translateY(-1px);
  box-shadow: 0 8px 24px rgba(201,123,138,0.2);
}

/* ── Hero ── */
.unauthorized-hero { text-align: center; padding: 20px 0 8px; margin-bottom: 48px; animation: fadeUp 0.7s ease both; }
.unauthorized-hero__eyebrow {
  display: inline-flex; align-items: center; gap: 10px;
  background: linear-gradient(135deg, rgba(201,123,138,0.2), rgba(122,171,142,0.15));
  border: 1px solid rgba(201,123,138,0.3); border-radius: 30px;
  padding: 8px 20px; margin-bottom: 32px; font-size: 13px;
  letter-spacing: 0.02em; color: var(--color-text);
}
.unauthorized-hero__eyebrow-dot {
  width: 6px; height: 6px; border-radius: 50%;
  background: var(--color-neon-rose); box-shadow: 0 0 8px var(--color-neon-rose);
  animation: pulse 2s infinite; flex-shrink: 0;
}
.unauthorized-hero__title {
  font-family: var(--font-display); font-size: clamp(38px, 6vw, 72px);
  font-weight: 300; line-height: 1.1; color: var(--color-text);
...
.unauthorized-hero__subtitle {
  ...
  max-width: 420px; margin: 0 auto 40px; line-height: 1.6; font-weight: 300;
}
.unauthorized-hero__cta-btn {
  ...
  box-shadow: 0 8px 32px rgba(201,123,138,0.35); text-transform: uppercase;
}
.unauthorized-hero__cta-btn:hover { transform: translateY(-3px); box-shadow: 0 16px 40px rgba(201,123,138,0.45); }

/* ── Цитата дня ── */
.unauthorized-daily-quote { margin: 52px 0; animation: fadeUp 0.7s 0.1s ease both; }
.unauthorized-daily-quote__label {
  font-size: 11px; letter-spacing: 0.22em; text-transform: uppercase;
  color: rgba(245,230,211,0.35); margin-bottom: 20px; text-align: center;
}
.unauthorized-daily-quote__card {
  display: flex; align-items: flex-start; gap: 20px; padding: 32px 40px;
  background: linear-gradient(135deg, rgba(74,25,66,0.5), rgba(45,15,42,0.7));
  border: 1px solid rgba(201,123,138,0.2); border-radius: 24px;
...
.unauthorized-daily-quote__text {
  ...
  color: var(--color-text); line-height: 1.4; margin-bottom: 12px;
}
.unauthorized-daily-quote__source {
  font-size: 12px; color: var(--color-rose);
  letter-spacing: 0.15em; text-transform: uppercase; opacity: 0.8;
}

/* ── Тебе понравится ── */
.unauth-hot { animation: fadeUp 0.7s 0.2s ease both; }
.unauth-hot__source {
  font-size: 12px; color: var(--color-rose); text-decoration: none;
  letter-spacing: 0.1em; text-transform: uppercase; opacity: 0.8; transition: opacity 0.2s;
}
.unauth-hot__source:hover { opacity: 1; text-decoration: underline; }
.unauth-hot__grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 16px; }
...
.unauth-card:hover { transform: translateY(-6px); border-color: rgba(201,123,138,0.35); box-shadow: var(--shadow-card); }

.unauth-card__cover {
  position: relative; aspect-ratio: 2/3; overflow: hidden;
  background: rgba(255,255,255,0.04);
}
.unauth-card__cover img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.4s ease; }
.unauth-card:hover .unauth-card__cover img { transform: scale(1.04); }

.unauth-card__overlay {
  position: absolute; inset: 0;
  background: linear-gradient(to top, rgba(45,15,42,0.95) 0%, transparent 55%);
}
.unauth-card__badges { position: absolute; top: 10px; left: 10px; right: 10px; display: flex; gap: 5px; }

.unauth-card__ext-btn {
  position: absolute; bottom: 10px; right: 10px;
  width: 30px; height: 30px; border-radius: 50%;
  background: rgba(201,123,138,0.85); border: none;
  display: flex; align-items: center; justify-content: center;
  color: #fff; opacity: 0; transform: scale(0.7);
  transition: var(--transition-fast); cursor: pointer;
}
.unauth-card:hover .unauth-card__ext-btn { opacity: 1; transform: scale(1); }

/* Рейтинг — пилюля поверх постера снизу-слева */
.unauth-card__rating {
  position: absolute; bottom: 10px; left: 10px;
  display: flex; align-items: center; gap: 4px;
  padding: 3px 8px; border-radius: 20px;
  background: rgba(30,10,28,0.75); backdrop-filter: blur(6px);
  border: 1px solid rgba(212,165,116,0.35);
  font-size: 11px; font-weight: 500; color: var(--color-gold, #d4a574);
  letter-spacing: 0.03em;
}

.unauth-card__info { padding: 12px 12px 14px; }
.unauth-card__info .card-title { font-size: 14px; margin-bottom: 5px; }

/* ── Skeleton ── */
.unauth-card--skeleton { pointer-events: none; }
.unauth-skel-box { aspect-ratio: 2/3; background: rgba(255,255,255,0.06); animation: unauth-pulse 1.5s ease-in-out infinite; }
.unauth-skel-line { border-radius: 6px; background: rgba(255,255,255,0.06); animation: unauth-pulse 1.5s ease-in-out infinite; }
@keyframes unauth-pulse { 0%,100% { opacity:0.4; } 50% { opacity:0.85; } }
...
.unauthorized-login-banner {
  ...
  backdrop-filter: blur(10px); animation: fadeUp 0.7s 0.35s ease both;
}
.unauthorized-login-banner__left { display: flex; align-items: center; gap: 18px; }
.unauthorized-login-banner__icon { font-size: 36px; }
.unauthorized-login-banner__title { font-family: var(--font-display); font-size: 22px; font-weight: 400; margin-bottom: 4px; color: var(--color-text); }
...
.unauthorized-login-banner__btn {
  ...
  letter-spacing: 0.06em; cursor: pointer; transition: var(--transition-normal);
  white-space: nowrap; box-shadow: 0 6px 20px rgba(201,123,138,0.3);
  text-transform: uppercase; flex-shrink: 0;
}
.unauthorized-login-banner__btn:hover { transform: translateY(-2px); box-shadow: 0 12px 30px rgba(201,123,138,0.4); }

/* ── Responsive ── */
@media (max-width: 1100px) { .unauth-hot__grid { grid-template-columns: repeat(4, 1fr); } }
@media (max-width: 860px) {
  .unauth-hot__grid { grid-template-columns: repeat(3, 1fr); }
...
@media (max-width: 640px) { .unauth-hot__grid { grid-template-columns: repeat(2, 1fr); } }
`;

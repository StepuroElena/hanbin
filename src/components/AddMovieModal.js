 ...
 * страна выпуска (опционально, дропдаун с поиском по названию).
 */

import { closeModal, injectModalCSS } from './LoginModal.js';
import { addMovie } from '../api/mock.js';
  ...
  return `
    ...
    <div class="hb-field">
      ...
      <div class="hb-chips" id="hb-addmovie-genre-chips">
        ${GENRE_KEYS.map(g => `<div class="hb-chip ${savedState.genre === g.value ? 'hb-chip--active' : ''}" data-value="${g.value}">${t(g.key)}</div>`).join('')}
      </div>
...
export function mountAddMovieContent(content, savedState = {}, { onAdded } = {}) {
  ...
  document.getElementById('hb-addmovie-genre-chips')?.addEventListener('click', (e) => {
    ...
    wrap.querySelectorAll('.hb-chip').forEach(c => c.classList.remove('hb-chip--active'));
  ...
  async function submit() {
    ...
    const { data, error } = await addMovie({ title, genre, country, year });
